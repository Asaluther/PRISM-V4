...
### 4.8 500M 判决：压缩曲线反转（v6.1 新增，启智 V100S）

d1280/24L、20K 步（164M tokens ≈ 328-497 t/p）、直承 5e-4、单 seed、fp16：

| | HYB 12+12（499.5M） | TF 24L（536.9M） |
|---|---|---|
| **Best PPL** | **39.55** | **750.35** |
| 轨迹 | 单调健康到底 | 2K 步最优 1024 → 平台期劣化 1182 → 爬回 750 |
| 训练事件 | ~700 次 fp16 跳过 + 1 次回滚（自愈） | 零 NaN（纯动力学塌方） |

**同协议优势 94.7%——上段的归零担忧被反转否定**：收窄曲线（36.4% → ~6.5%）
在越过 TF 的甜点区后反转。两种天花板的对照是本节核心：330→500M 同预算下
HYB 撞**饱和墙**（39.75→39.55——164M tokens 预算下混合架构的最优规模就在
此区间，增益止步但无损），TF 撞**灾难墙**（17.9× 劣化）——**数据稀缺前沿上，
混合架构的规模天花板是优雅的，纯 Transformer 的天花板是灾难性的**。
TF 的零 NaN 塌方与 §4.6 纯 PCN 崩溃互为镜像（角色互换、教训对称）；
B4 的数据比规律（固定规模下稀缺放大混合优势）以极端形态兑现。

诚实边界：TF@2.5e-4 夹逼因积分砍除（354M 已证该档大劣 52%——可用窗口
即使存在也窄；积分恢复后第一优先补强）；双臂单 seed；HYB 含一次紧急
lr 降档（39.55 略保守）。

数据：`results/wt_516m_hyb_ng_lr5e-4_s0/`、`results/wt_553m_tf_lr5e-4_s0/`、`results/SCALE_500M_VALIDATION.md`
...
### 7.8 PEFT 网格与适应动力学（v6.1 新增，C 线补强）

**22 格 PEFT 网格**（评审 W2 规格，3 ckpt-seed × 3 用户/格，`results/v7/peft_grid.json`）：

- **HYB 全 22 格为正**（+14.9~+80.7）——跨方法（LoRA/bitfit）、跨 rank（4/8/16）、
  跨 α（16/32）、跨 lr（1e-4~1e-3）的网格级零配置稳健性证据
- **v6.0 的「LoRA-TF 结构性不可行」修正**：在 lr 1e-4（此前未测档位）下转正
  （+71.3）——TF 的可用配置空间是稀疏岛，不是全域不可行
- **HYB+LoRA@1e-4 = +80.7 是全场最优单发**，超过 TF+bitfit 的 +76.6

**适应动力学纵向日志**（评审 W4/Q2 响应，`results/v7/adapt_dynamics.json`）：

三架构头梯度范数在 300 步适应中的轨迹分化：

| 架构 | step10 → step300 | 倍率 | 含义 |
|---|---|---|---|
| **混合 h2e4** | 10.3 → 68.6 | **×6.6** | 独有的「启动→加速→过热」三段动力学 |
| 纯 PCN | 0.124 → 0.135 | ×1.09 | 全 24 层从头活跃，无对比度 |
| TF | 0.328 → 0.026 | ×0.08 | 标准收敛衰减 |

**误差正反馈的首个纵向微观证据**：混合架构的 PCN 头梯度在适应过程中增长
6.6×，对应记忆过程加速——而纯 PCN 平稳、TF 头衰减。

**梯度阻尼干预（否定性结果）**：额外钳制头梯度（0.3/0.1 档）**没有**减轻
遗忘——AdamW 动量使钳制退化为步长缓释。遗忘不是「梯度过大」问题，真正
的旋钮是步数预算（step50 结论），不是梯度幅度——机制解释收窄到「参数
重新征用」。

**seq-128 敏感性**（评审 Q5 最小检查，`results/wt_09*m_seq128_s0/`）：

| | seq256 基线 | seq128 | 变化 |
|---|---|---|---|
| HYB | 63.18 | 88.88 | +40.7% |
| TF | 99.34 | 194.36 | +95.5% |
| **优势** | **+36.4%** | **+54.3%** | **稀缺放大 +18pp** |

短 seq 加剧数据稀缺——混合优势放大，与 B4 数据比规律方向一致。倒 U 的
定性结构对 seq 不变。。
...
## 8B. 真实用户数据与多域共存（v6.1 新增，A 线）

### 8B.1 真实用户数据试点

| 臂 | 冻结 PPL | 适应后 | 改善 | 数据 |
|---|---|---|---|---|
| **R 真实用户** | 187 | **62** | **+66.6%** | 手写中文日记（611 字→1311 tok→4+1 序列） |
| S 结构化噪声 | 256 | 94 | +63.1% | 生成 LLM 运维笔记（1.25M 字中英混合） |
| 对照 TS | 1740 | 225 | +87.1% | 英文模拟用户 |

英文预训练 96M 混合模型 50 步 CPU 适应真实中文日记 → +66.6%——终端个性化
跨语言成立。改善低于 TS（66.6 vs 87.1%）因中文域更远+起点更低。生成从
英文乱码变为纯中文字符（域已学）但不连贯——中文终端应用应配中文底座。

### 8B.2 多主题交替流（150 批 ABAB）

| 指标 | h2e4 | 纯 PCN |
|---|---|---|
| 全程增益 | **+82.4%** | +53.7% |
| A/B 域增益 | +82.5/+82.2 | +49.9/+57.5 |
| **切换成本** | **0.0%** | 0.0% |
| A 域终末探针 | **+79.8%** | **-6.1%（负）** |
| 源域遗忘 | 12× | 45× |

**零切换成本 + 混合双域均衡 + 纯 PCN 跨域干扰（B 学覆写 A）**——终端多
用户/多域共存场景下混合架构是唯一可行选择。

### 8B.3 int4 修正

v7_demo 的「int4 适应损失 10pp」是 tokenizer 往返伪影。严格三臂（同检查点
同协议直接 token ID）：全 fp32 +87.8% / 全 int4 **+87.7%** / 两段式 +88.2%
——全 int4 即可直接适应，最优体积 132 MB。
