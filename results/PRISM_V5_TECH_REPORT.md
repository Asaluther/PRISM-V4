...
> 2026-09-17 v5.1 | 覆盖 2026-09-07 至 09-17 的完整研究记录（v4 新增：V7 终端形态——稀疏激活与冷启动兼容、CPU 端侧部署、安全框架、第五次测量偏差修正；v5 新增：启智 V100 上的 100M 规模验证；**v5.1 勘误：TF lr 扫描显示最优超参随规模漂移，v5 版「+40.1%」结论系伪影已撤回，规模对比结论悬置待对称扫描**）
...
1. **规模-数据比规律**：误差流变体（no_gating）相对 Transformer 的优势随 tokens/parameter 比下降呈**倒 U 形**——峰值出现在 ≈1000（难域 WikiText-103 上 +60.8%），数据充裕后被反超（-64.9%）。该规律成功预测了一个未测数据点（WT d512 实测 +37.7%，落在预测带 [+30%, +55%]）。**规模外推（v5 新增，v5.1 勘误）**：最优超参随规模漂移——63M 扫出的 TF 最优 lr 1e-3 在 101M 上劣于调优值 2.4 倍（5e-4 → PPL 99.7）；v5 版「优势在 100M 保持（+40.1%）」系 TF 未调参的伪影，**已撤回**。当前调优 TF（99.7）领先 PCN@1e-4（150.2），但 PCN 的 lr 同样未在本规模重扫，**对比结论悬置**，待对称扫描后判定（§4.6）。
...
### 4.6 规模验证与超参漂移勘误（v5 新增，v5.1 勘误，启智 V100）

24L/d=512 双架构、除架构与 lr 外全对齐：warmup 2000、eff batch 32、seq 256、10K steps = 82M tokens、同 tokenizer 与数据切分、n_heads=4 一致、NaN 恢复机制双启用（零触发）。完整数据与轨迹表见 `results/SCALE_090M_VALIDATION.md`。

| 架构 | lr | Best PPL |
|---|---|---|
| TF 101.52M | **5e-4** | **99.66** |
| TF 101.52M | 1e-3 | 236.53 / 250.85（运行间方差 6.0%） |
| TF 101.52M | 2e-3 | 452.46 |
| PCN 90.48M | 1e-4 | 150.13 / 150.37（方差 0.16%） |

- **v5 初步结论已撤回**：首跑沿用双方 63M 规模扫描最优（PCN 1e-4 / TF 1e-3）得到「PCN -40.1%」。TF lr 扫描揭示**最优超参随规模漂移**——101M 上 TF 最优 lr ≤5e-4，1e-3 的 PPL 是调优值的 2.4 倍。此为本系列第 6 起系统性测量偏差（LESSONS #2 同类偏差的规模升级复发），由预设的「先扫描后主张」协议捕获
- **当前状态（悬置）**：调优 TF（99.66）领先 PCN@1e-4（150.25）；但 PCN 的 lr 同样未在本规模重扫（小规模最优 5e-4，101M 因稳定性曾降至 1e-4）。「100M 上谁占优」的最终判定待对称扫描（TF 3e-4 / PCN 3e-4、5e-5）完成后给出
- 复现的观察：TF@1e-3 运行间方差 6.0% vs PCN 0.16%——「误差流稳定性优势」在 100M 复现；TF@5e-4 末端仍改善（~5.7%/2k steps），真最优可能 <5e-4；lr 敏感度：2× lr → 2.4× PPL
- 流程教训（待 scan2 后并入 §3 方法学条款）：**超参最优值必须在新规模各自重扫，跨规模沿用旧最优即使有历史依据也不可接受**
