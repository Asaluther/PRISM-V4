...
> 2026-09-16 v5 | 覆盖 2026-09-07 至 09-16 的完整研究记录（v4 新增：V7 终端形态——稀疏激活与冷启动兼容、CPU 端侧部署、安全框架、第五次测量偏差修正；v5 新增：启智 V100 上的 100M 规模验证）
...
1. **规模-数据比规律**：误差流变体（no_gating）相对 Transformer 的优势随 tokens/parameter 比下降呈**倒 U 形**——峰值出现在 ≈1000（难域 WikiText-103 上 +60.8%），数据充裕后被反超（-64.9%）。该规律成功预测了一个未测数据点（WT d512 实测 +37.7%，落在预测带 [+30%, +55%]）。**规模外推（v5 新增）**：10× 规模上成立——24L，PCN 90.5M vs TF 101.5M、82M tokens（V100），峰区内（905 tokens/param）PCN PPL 150.2 vs TF 250.9（**-40.1%，且少 10.9% 参数**）。
...
### 4.6 规模验证：100M 上规律成立（v5 新增，启智 V100）

审稿人最常要求的上尺度数据点。24L/d=512 双架构、除架构外全对齐：各自扫描出的最优 lr（PCN 1e-4 / TF 1e-3）、warmup 2000、eff batch 32、seq 256、10K steps = 82M tokens、同 tokenizer 与数据切分、n_heads=4 一致、NaN 恢复机制双启用（均零触发）。完整分析与轨迹表见 `results/SCALE_090M_VALIDATION.md`。

| | PCN (no_gating) | Transformer (SDPA) |
|---|---|---|
| 参数量 | 90.48M | 101.52M |
| Best PPL | **150.13 / 150.37**（两次运行，差 0.16%） | 250.85 |
| tokens/param | 905 | 807 |
| 训练吞吐 | ~38.5K tok/s | ~44.2K tok/s |

- **PCN 少 10.9% 参数、PPL 低 40.1%**（TF 比 PCN 高 67%）。905 tokens/param 落在 §4.2 倒 U 峰区（小规模峰位 ≈1025、+60.8%；此处实测 +40.1%——方向与峰位一致，幅度略低，属单点 24L 大模型 vs 小规模 5-seed 曲线的合理量级差异）
- **轨迹复现数据轴规律**：TF/PCN 的 PPL 比值从 step 1000 的 2.05× 逐步收窄至 1.67×——「优势随数据积累衰减」在大规模同向复现
- **数据效率外推（非实测，末段幂律拟合 a≈0.28）**：TF 追平 150 PPL 需 ≈6× tokens（~508M，~3.2h V100）
- 诚实边界：TF 为单 seed（PCN 重复运行显示该规模方差 ~0.16%）；TF lr 沿用 63M 扫描最优、未在 101M 重扫；n_heads=4/d_head=128 两架构一致但非主流配置，对 TF 绝对性能或有少量影响
