...
> 2026-09-17 v5.4 | 覆盖 2026-09-07 至 09-17 的完整研究记录（v4 新增：V7 终端形态——稀疏激活与冷启动兼容、CPU 端侧部署、安全框架、第五次测量偏差修正；v5 新增：启智 V100 上的 100M 规模验证；v5.1 勘误：超参随规模漂移，「+40.1%」伪影撤回；v5.2 终判：调优 TF 预训练领先 33.7%；v5.3：V8-A 稳定化解锁证伪（失稳系优化动力学非数值精度），PCN lr 网格闭合；**v5.4：90M 冷启动判决——适应优势保持且放大（PCN +62.5% vs TF -127.4%，微调 lr 扫描排除伪影），确立混合架构路径（§7.3）**）
...
5. **终端适应的真实边界（v2.2 新增；v5.4 规模验证）**：误差流的适应优势属于**跨域/冷启动**——单发少量数据微调 +58.5% vs 公平 TF +5.4%，真实流式（一次性逐批）+24.0% vs TF -80.9%；且**该优势在 90-101M 规模上保持并放大：PCN +62.5%（对微调 lr 鲁棒）vs TF -127.4%（所有微调 lr 下均负）**（§7.3）；但**域内个性化无适应空间**（24 格校准网格双架构全负），且匹配适应水平下 PCN 的遗忘**更多**而非更少（ΔW 小 ≠ 功能稳定——权重约束重定性为「参数经济性/高函数敏感度」）。部署准则：冷启动用 PCN 边用边学，域内已收敛则冻结。
...
**90M 规模外推（v5.4 新增，判决性）**：在 §4.6 的 90-101M 对称调优检查点上重跑本协议（本地复训，跨 GPU 复现 <1.4%）：**PCN +62.5%（3 用户全正，+41~+83）vs TF -127.4%（3 用户全负，微调后 PPL 暴涨）**——优势不仅保持且放大（小规模 +58.5% vs +5.4%）。微调 lr 敏感性扫描（5e-4/1e-4/5e-5 对称双臂）排除超参伪影：PCN 三档全部 >+50%，TF 三档全部为负。机制：误差驱动更新在少样本域适应中天然自限，标准 AdamW 微调已收敛大模型则灾难性过拟合。**该性质经受住了规模检验，是终端在线学习的核心依据；与 §4.6 预训练终判不矛盾——预训练属数据充裕区（TF 区），个性化属数据稀缺区（PCN 区），支持混合架构路径。**数据：`results/v7/coldstart_090m{,_ftlr}.json`
