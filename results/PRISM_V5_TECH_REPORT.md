...
> 2026-09-18 v5.9 | 覆盖 2026-09-07 至 09-18 的完整研究记录（v4 新增：V7 终端形态——稀疏激活与冷启动兼容、CPU 端侧部署、安全框架、第五次测量偏差修正；v5 新增：启智 V100 上的 100M 规模验证；v5.1 勘误：超参随规模漂移，「+40.1%」伪影撤回；v5.2 终判：调优 TF 预训练领先 33.7%；v5.3：V8-A 稳定化解锁证伪，PCN lr 网格闭合；v5.4：90M 冷启动判决，确立混合架构路径；v5.5：90M 流式判决，双协议闭合；v5.6：混合架构最小原型判决成立；v5.7：3-seed 复现全判据命中；v5.8：阶段②敏感性扫描，帕累托前沿 + 主干锚定机制；**v5.9：阶段③端侧链路——96M 混合架构纯 CPU 验证 4/4 判据命中，冷启动 20s 达峰 +85%（协议发现：适应峰值在 step50，终点测法低估 25pp）（§9.3）**）
> 关键词：非 Transformer 架构、预测编码、样本效率、因果性、消融方法学
> **代码与数据已开源**：https://github.com/Asaluther/PRISM-V4
> 单卡 RTX 4080 16GB，PyTorch 2.14，≈170 有效训练 run（含 V6/V7 系列）（另含 75 项作废归档；口径详见附录 B）；规模验证 run 于启智社区 V100S-32GB 完成（24L/90-101M，共 3 次）
...
1. **规模-数据比规律**：误差流变体（no_gating）相对 Transformer 的优势随 tokens/parameter 比下降呈**倒 U 形**——峰值出现在 ≈1000（难域 WikiText-103 上 +60.8%），数据充裕后被反超（-64.9%）。该规律成功预测了一个未测数据点（WT d512 实测 +37.7%，落在预测带 [+30%, +55%]）。**规模外推（v5 新增，v5.2 终判）**：最优超参随规模漂移（63M 扫出的 TF 最优 1e-3 在 101M 劣于调优值 2.4 倍；v5 版「+40.1%」系 TF 未调参伪影，已撤回）。双方对称扫描闭合后：**100M（24L，82M tokens，~900 tokens/param）上调优 TF（99.7 @5e-4）领先调优 PCN（150.2 @1e-4）33.7%——小规模低数据区优势未随规模延续**。失败模式指向**稳定性天花板而非容量天花板**：PCN@3e-4 早期轨迹全场最佳（step 2000 PPL 291 vs TF 最好 766）后在 fp16 激活溢出中崩溃（权重非 NaN，恢复机制盲区）——稳定化（bf16/qk-norm）是明确的解锁方向（§4.6）。
...
5. **终端适应的真实边界（v2.2 新增；v5.4/v5.5 规模验证）**：误差流的适应优势属于**跨域/冷启动**——单发少量数据微调 +58.5% vs 公平 TF +5.4%，真实流式（一次性逐批）+24.0% vs TF -80.9%；且**该优势在 90-101M 规模上经双协议验证均保持并放大：单发 +62.5% vs -127.4%，流式 +34.3% vs -139.5%，PCN 六档适应 lr 全正、TF 六档全负**（§7.3）；但**域内个性化无适应空间**（24 格校准网格双架构全负），且匹配适应水平下 PCN 的遗忘**更多**而非更少（ΔW 小 ≠ 功能稳定——权重约束重定性为「参数经济性/高函数敏感度」）。部署准则：冷启动用 PCN 边用边学，域内已收敛则冻结。
...
**核心结论**：PCN 是「低数据/参数比区间的优势架构」——其可兑现场景是**跨域冷启动适应**（换域/新用户冷启动，经 90M 规模判决验证），而非域内风格微调或大规模预训练（100M 对称调优下 TF 领先 33.7%）；其能力是不可移植的组合性质，必须在非 Transformer 骨架上整体产生。**终端形态的路径是混合架构：TF 类骨干（预训练）+ 误差流适应层（个性化）——其最小原型（12TF+12PCN，96M）已验证成立：预训练 PPL 63.4（比纯 TF 好 36%），单发冷启动 +54.6%，流式 +43.0%（lr 降档后与纯 PCN 持平）（§7.6）。**
...
### 7.6 混合架构最小原型：一个模型同时拿到两种能力（v5.6 新增）
...
- **主结果（意外但机制连贯）**：混合架构以更少参数（96M vs 101.5M）在预训练上
  大幅超过纯 TF——82M tokens ≈ 853 tokens/param 正处数据稀缺峰区，纯 TF 无法
  利用误差流加速、纯 PCN 被稳定性压住，12+12 各取所长。与 2D 定律自洽而非矛盾
- **流式阈值移动**：hyb 的 PCN 头流式稳定阈值比纯 PCN 低 2-5×（2.5e-4 档：
  +41.9 vs -47.9），假设机制为已收敛主干特征上误差更尖锐；5e-5 档恢复纯 PCN
  最佳水平。部署含义：在线学习步长按头保守配置
- 诚实边界：~~全部单 seed~~ **已关闭——3-seed 复现（v5.7）全判据命中**：预训练
  HYB 63.18±0.22（seed 方差三者最小）；单发 +60.3±4.3%；流式@5e-5 +26.1~+43.0%
  （阈值移动跨 seed 复现，部署固化保守步长 5e-5）。PCN 流式九格全正、TF 九格全负
  的符号矩阵亦 seed 稳健。剩余边界：hybrid 自身 lr 方案未扫描；HYB@5e-5 的 seed
  方差大于纯 PCN（机制待查）。完整数据：`results/SEED_REPLICATION.md`

**敏感性扫描（v5.8 新增，阶段②）**：分割比与头 lr 两轴四变体（`results/PHASE2_SENSITIVITY.md`）：
分割比轴预训练严格单调（16+8: 54.4 → 12+12: 63.2 → 8+16: 74.5），但 **PCN 头 <12 层时单发适应塌陷**（8 层头仅 +18.9%，流式尚存 +50.8%）；头 lr 轴呈现清晰**预训练-适应帕累托前沿**——头 5e-5 预训练微升而适应全面变差（「温和预训练让头变钝」），头 2e-4 预训练劣化 20%（75.66）但适应全场最强（单发 +62.5、流式 +71.1/@1e-4 +60.2，**流式阈值移动消失**）。**机制发现（V8 答案的架构形式）**：纯 PCN@24L 在 2e-4 软发散（185→崩），而 12 层 PCN 头在 TF 主干上同 lr 不崩且保持最强适应——**主干锚定阻断了误差正反馈的自毁路径，「快速学习区间」与「自毁区间」在混合结构中部分解耦**（当初设想的 qk-norm 修复可能并非必需）。部署结论：通用默认 12+12 头 1e-4；终端在线学习优先场景候选 12+12 头 2e-4（单 seed，待复现）。
