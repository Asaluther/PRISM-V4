...
> 2026-09-17 v5.5 | 覆盖 2026-09-07 至 09-17 的完整研究记录（v4 新增：V7 终端形态——稀疏激活与冷启动兼容、CPU 端侧部署、安全框架、第五次测量偏差修正；v5 新增：启智 V100 上的 100M 规模验证；v5.1 勘误：超参随规模漂移，「+40.1%」伪影撤回；v5.2 终判：调优 TF 预训练领先 33.7%；v5.3：V8-A 稳定化解锁证伪（失稳系优化动力学非数值精度），PCN lr 网格闭合；v5.4：90M 冷启动判决——适应优势保持且放大，确立混合架构路径；**v5.5：90M 流式判决——双协议闭合（单发 +62.5%/-127.4%，流式 +34.3%/-139.5%，六档适应 lr 全正 vs 全负）（§7.3）**）
...
**核心结论**：PCN 是「低数据/参数比区间的优势架构」——其可兑现场景是**跨域冷启动适应**（换域/新用户冷启动，经 90M 规模判决验证），而非域内风格微调或大规模预训练（100M 对称调优下 TF 领先 33.7%）；其能力是不可移植的组合性质，必须在非 Transformer 骨架上整体产生。**终端形态的诚实路径是混合架构：TF 类骨干（预训练）+ 误差流适应层（个性化）——两个组件各自站在被验证的优势区。**
