...
> 2026-09-17 v5.6 | 覆盖 2026-09-07 至 09-17 的完整研究记录（v4 新增：V7 终端形态——稀疏激活与冷启动兼容、CPU 端侧部署、安全框架、第五次测量偏差修正；v5 新增：启智 V100 上的 100M 规模验证；v5.1 勘误：超参随规模漂移，「+40.1%」伪影撤回；v5.2 终判：调优 TF 预训练领先 33.7%；v5.3：V8-A 稳定化解锁证伪，PCN lr 网格闭合；v5.4：90M 冷启动判决，确立混合架构路径；v5.5：90M 流式判决，双协议闭合；**v5.6：混合架构最小原型判决——12TF+12PCN/96M 预训练 PPL 63.4（比纯 TF 好 36%）+ 单发 +54.6% + 流式降档 +43.0%，判据 2/3 直接过 + 1 降档过（§7.6）**）
...
|---|---|---|---|---|
| 预训练 PPL | 99.66 | 150.25 | **63.42** | ✅ ≤130 超额（**比纯 TF 好 36.3%**） |
| 单发冷启动 | -49.8% | +75.9% | **+54.6%**（3 用户全正） | ✅ ≥+30% |
| 流式在线增益 @1e-4 | -139.5% | +34.3% | -6.7% | ❌ |
| 流式在线增益 @5e-5 | -56.3% | +44.3% | **+43.0%** | ✅ 降档命中 |
...
- 诚实边界：全部单 seed（预训练差 36% 裕度大但需 3-seed 复现）；hybrid 自身
  lr 方案未扫描（组件级最优是设计选择）；单发协议跨会话方差显著（TF 臂
  -49.8↔-127.4，符号量级稳定）
