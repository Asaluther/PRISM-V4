...
> 2026-09-17 v5.2 | 覆盖 2026-09-07 至 09-17 的完整研究记录（v4 新增：V7 终端形态——稀疏激活与冷启动兼容、CPU 端侧部署、安全框架、第五次测量偏差修正；v5 新增：启智 V100 上的 100M 规模验证；v5.1 勘误：超参随规模漂移，「+40.1%」伪影撤回；**v5.2 终判：双方对称扫描闭合，100M 上调优 TF 领先 33.7%——小规模优势未延续，失败模式指向稳定性天花板（§4.6）**）
...
| **PCN 90.48M** | **1e-4** | **150.13 / 150.37** | **最优（两侧夹逼，方差 0.16%）** |
| PCN 90.48M | 5e-5 | 207.17 | 稳定 |
| PCN 90.48M | 3e-4 | 290.97* | 失稳崩溃 |
...
- **稳定性天花板（而非容量天花板）**：PCN@3e-4 早期轨迹全场最佳（step 1000/2000：469/291；同期 TF 最好 1244/766，PCN@1e-4 为 688/383），step ~2500 起劣化，step 5292 起进入 fp16 激活溢出死循环——eval 在 fp32 下仍正常（step 6000 输出 352.06），权重非 NaN，NaN 恢复机制对「激活溢出、权重干净」模式存在盲区。可用 lr 上限随深度收紧（小规模最优 5e-4 → 24L 稳定最优被压至 1e-4），压缩了 PCN 的调优空间。**解锁方向：bf16、qk-norm、连续 NaN-loss 触发恢复**
